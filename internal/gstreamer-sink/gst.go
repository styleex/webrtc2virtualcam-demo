...
func CreatePipeline(payloadType webrtc.PayloadType, codecName string) *Pipeline {
	...
	case "vp8":
		pipelineStr += fmt.Sprintf(", payload=%d, encoding-name=VP8-DRAFT-IETF-01 ! rtpvp8depay ! decodebin ! %s", payloadType, videoOutput)
	case "opus":
		pipelineStr += fmt.Sprintf(", payload=%d, encoding-name=OPUS ! rtpopusdepay ! decodebin ! autoaudiosink", payloadType)
	case "vp9":
		pipelineStr += fmt.Sprintf(" ! rtpvp9depay ! decodebin ! %s", videoOutput)
	case "h264":
		pipelineStr += fmt.Sprintf(" ! rtph264depay ! decodebin ! %s", videoOutput)
	case "g722":
